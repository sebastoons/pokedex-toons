...
// Lista de generaciones, incluyendo la especial
...
// --- ¡NUEVA FUNCIÓN INTELIGENTE PARA LAS IMÁGENES! ---
const getPokemonImageUrl = (pokemon) => {
    // Si el Pokémon es especial, usa la ruta de la imagen local
    if (pokemon.isSpecial) {
        return pokemon.imageUrl;
    }
    // Si no, construye la URL de la API como siempre
...
function PokemonBattleSelector({ pokemonList }) {
    ...
    // Lógica de filtrado que ya reconoce la generación especial
    ...
    return (
        <div className="battle-selector-container">
            <Link to="/" className="pokemon-link-button back-to-pokedex-top">&lt; Volver a la Pokédex</Link>
                    ...
                    <div className="selected-pokemon-slot" key={index}>
                        ...
                        {pokemon ? (
                            <div className="selected-pokemon-card-preview">
                                ...
                                <img src={getPokemonImageUrl(pokemon)} alt={pokemon.name} className="pokemon-preview-image" />
            ...
            <Link to="/" className="pokemon-link-button back-to-pokedex">Volver a la Pokédex</Link>
        </div>
    );
}
